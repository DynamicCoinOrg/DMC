...
export DYNAMICCOINCLI=${BUILDDIR}/qa/pull-tester/run-dynamiccoin-cli
export DYNAMICCOIND=${REAL_DYNAMICCOIND}
...
if [ "x${ENABLE_DYNAMICCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and dynamiccoind must all be enabled"
fi
